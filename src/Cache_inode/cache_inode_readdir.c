                              ...
                              struct fsal_cookie *cookie)
{
        ...
        status = dir_hdl->ops->lookup(opctx, dir_hdl, name, &entry_hdl);
                             ...
                             cache_inode_status_t *status)
{
  ...
  *status = CACHE_INODE_SUCCESS;
  ...
  fsal_status = directory->obj_handle->ops->readdir(req_ctx,
                    ...
                    cache_inode_status_t *status)
{
     ...
     *status = CACHE_INODE_SUCCESS;
     ...
     /* cache_inode_lock_trust_attrs can return an error, and no lock will be
        acquired */
     *status = cache_inode_lock_trust_attrs(req_ctx, directory);
