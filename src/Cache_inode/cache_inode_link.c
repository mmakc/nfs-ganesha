                                      ...
                                      cache_inode_status_t *status)
{
...
#endif /* _USE_NFS4_ACL */
     fsal_status = entry->obj_handle->ops->link(req_ctx, entry->obj_handle,
                                                dest_dir->obj_handle,
                                                name);
     ...
     if (FSAL_IS_ERROR(fsal_status)) {
          *status = cache_inode_error_convert(fsal_status);
          if (fsal_status.major == ERR_FSAL_STALE) {
               ...
               fsal_status = dest_dir->obj_handle->ops->getattrs(req_ctx,
                                                                 dest_dir->obj_handle,
								 &attrs);
               if (fsal_status.major == ERR_FSAL_STALE) {
     ...
     *attr = entry->obj_handle->attributes;
     ...
     cache_inode_refresh_attrs(req_ctx, dest_dir);
