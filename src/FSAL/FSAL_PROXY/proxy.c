...
static proxyfs_specific_initinfo_t default_pxy_params = {
        ...
        .hdlmap.nb_handles_prealloc = 16384,
        .hdlmap.nb_db_op_prealloc = 1024,
#endif
};
                 ...
                 fsal_init_info_t *info, const char *name)
{
        ...
        if(!strcasecmp(key, "Srv_Addr")) {
                ...
                init_info->hdlmap.nb_handles_prealloc = (unsigned int)atoi(val);
        } else if(!strcasecmp(key, "HandleMap_Nb_DB_Operations_Prealloc")) {
                init_info->hdlmap.nb_db_op_prealloc = (unsigned int)atoi(val);
#endif
        } else {
                LogCrit(COMPONENT_CONFIG,
